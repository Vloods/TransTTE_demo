...
class GraphPredictionConfig(FairseqDataclass):
    ...
    mode: str = field(
            default="fit",
            metadata={"help": "fit || predict || predict_tte"},
        )
...
@dataclass
class GraphPredictionLinkConfig(GraphPredictionConfig):
    pass
...
@register_task("graph_prediction_link", dataclass=GraphPredictionLinkConfig)
class GraphPredictionLinkTask(GraphPredictionTask):
    """
    Graph prediction (classification or regression) task.
    """
    ...
    def train_step(
        ...
        model.train()
        model.set_num_updates(update_num)
        # print('batched_data:', self.batched_data)
        # print('sample x',sample["net_input"]["batched_data"])
        # print('sample x',sample["net_input"]["batched_data"]["x"].size())
        # print('sample',sample["net_input"]["batched_data"])
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss, sample_size, logging_output = criterion(model, sample, batched_data = self.batched_data)
        if ignore_grad:
            loss *= 0
